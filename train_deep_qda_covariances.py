...
from lda import SimplexQDAHead
...
class DeepQDA(nn.Module):
    def __init__(self, C, D, in_channels, covariance_type):
        super().__init__()
        self.encoder = Encoder(D, in_channels)
        self.head = SimplexQDAHead(C, D, covariance_type=covariance_type)
...
def train_one_run(dataset_name, C, D, in_channels, covariance_type, device, epochs, train_ld, test_ld, optimize_head):
    model = DeepQDA(C=C, D=D, in_channels=in_channels, covariance_type=covariance_type).to(device)
...
def main():
    parser = argparse.ArgumentParser(description="Train Deep QDA with multiple covariance types.")
    parser.add_argument("--data-root", default="./data", help="Dataset root directory.")
    parser.add_argument("--output", default="./results/deep_qda_covariances.json", help="JSON output file.")
